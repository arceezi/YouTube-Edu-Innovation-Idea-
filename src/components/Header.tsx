...
export const Header: React.FC<HeaderProps> = ({
  ...
  return <header className="sticky top-0 z-50 bg-[#0F0F0F] border-b border-[#272727]">
      <div className="flex items-center justify-between h-14 px-4">
        {/* Left section */}
        <div className="flex items-center">
          <button onClick={onMenuClick} className="p-2 mr-2 rounded-full hover:bg-[#272727]">
            <MenuIcon size={20} />
          </button>
          <Link to="/" className="flex items-center">
            <svg className="w-20 h-6" viewBox="0 0 90 20">
              <g viewBox="0 0 90 20">
        ...
        <div className="hidden md:flex flex-1 max-w-2xl mx-4">
          <div className="flex w-full">
            <div className="relative flex-1">
              <input type="text" placeholder="Search" className="w-full h-10 px-4 py-2 bg-[#121212] border border-[#303030] rounded-l-full focus:outline-none focus:border-[#1c62b9]" />
            </div>
            <button className="flex items-center justify-center h-10 w-16 bg-[#222222] border border-[#303030] border-l-0 rounded-r-full hover:bg-[#272727]">
              <SearchIcon size={20} />
            </button>
            <button className="ml-4 p-2 bg-[#181818] rounded-full hover:bg-[#272727]">
              <div size={20} />
            </button>
          </div>
        </div>
        {/* Right section */}
        <div className="flex items-center">
          <button className="p-2 rounded-full hover:bg-[#272727] md:mr-2">
      ...
      <div className="flex overflow-x-auto scrollbar-hide px-4 py-2">
        <button onClick={() => setActiveTab('home')} className={`px-3 py-1.5 mr-3 rounded-lg text-sm whitespace-nowrap ${activeTab === 'home' ? 'bg-white text-black' : 'bg-[#272727] hover:bg-[#3F3F3F]'}`}>
          All
        </button>
        <button onClick={() => setActiveTab('learning')} className={`px-3 py-1.5 mr-3 rounded-lg text-sm whitespace-nowrap ${activeTab === 'learning' ? 'bg-white text-black' : 'bg-[#272727] hover:bg-[#3F3F3F]'}`}>
          Learning
        </button>
        <button className="px-3 py-1.5 mr-3 rounded-lg text-sm whitespace-nowrap bg-[#272727] hover:bg-[#3F3F3F]">
          Music
        </button>
        <button className="px-3 py-1.5 mr-3 rounded-lg text-sm whitespace-nowrap bg-[#272727] hover:bg-[#3F3F3F]">
          Gaming
